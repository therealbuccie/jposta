...
export function EmployeeWorkspaceClient() {
  ...
  return (
    <div className="h-dvh min-h-0 overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(147,197,253,0.45),transparent_30%),radial-gradient(circle_at_80%_10%,rgba(221,214,254,0.45),transparent_34%),linear-gradient(135deg,#edf7ff_0%,#f8fbff_45%,#eaf3ff_100%)] text-slate-950">
      ...
      <div className="relative z-10 flex h-dvh min-h-0 flex-col gap-3 overflow-hidden p-2 sm:p-3 lg:p-3">
        <header className="grid gap-3 lg:grid-cols-[15rem_minmax(0,40rem)_auto] xl:grid-cols-[15.5rem_minmax(32rem,40rem)_auto]">
          <div className="hidden items-center gap-3 rounded-[1.1rem] border border-white/70 bg-white/42 px-3 py-2 shadow-[0_18px_60px_rgba(37,99,235,0.12)] backdrop-blur-2xl lg:flex"><LogoMark /><div className="min-w-0"><p className="truncate text-sm text-slate-500">Mailbox</p><p className="truncate text-base font-semibold text-blue-950">{workspace}</p></div></div>
          <div className="flex items-center gap-3 rounded-[1.35rem] border border-white/70 bg-white/58 p-2 shadow-[0_20px_70px_rgba(30,64,175,0.14)] backdrop-blur-2xl">
            <button aria-label="Open folders" className="inline-flex h-9 w-9 items-center justify-center rounded-2xl border border-white/70 bg-white/70 text-blue-900 shadow-sm lg:hidden" onClick={() => setSidebarOpen(true)} type="button"><Menu className="h-5 w-5" aria-hidden="true" /></button>
            <Search className="ml-1 h-5 w-5 shrink-0 text-blue-900" aria-hidden="true" />
            <input aria-label="Search emails, contacts, files" className="h-9 min-w-0 flex-1 bg-transparent text-base text-blue-950 outline-none placeholder:text-slate-500" onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && token) void loadMessages(token, activeFolder, 1, search, false); }} placeholder="Search emails, contacts, files..." value={search} />
          ...
          <div className="hidden items-center justify-end gap-3 lg:flex"><TopIconButton label="Theme"><Moon className="h-5 w-5" aria-hidden="true" /></TopIconButton><TopIconButton label="Notifications"><Bell className="h-5 w-5" aria-hidden="true" /></TopIconButton><TopIconButton label="Settings"><Settings className="h-5 w-5" aria-hidden="true" /></TopIconButton><div className="flex min-w-[13.5rem] items-center gap-2 rounded-[1.1rem] border border-white/70 bg-white/50 px-3 py-1.5 shadow-[0_18px_60px_rgba(30,64,175,0.12)] backdrop-blur-2xl"><Avatar name={displayName} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-blue-950">{displayName}</p><p className="truncate text-xs text-slate-500">{mailbox || "Mailbox"}</p></div><ChevronDown className="h-4 w-4 text-blue-900" aria-hidden="true" /></div></div>
        </header>
        <main className="grid min-h-0 flex-1 gap-3 overflow-hidden lg:grid-cols-[15rem_minmax(24rem,28rem)_minmax(0,1fr)] xl:grid-cols-[15.5rem_minmax(25rem,29rem)_minmax(0,1fr)]">
          <aside className={`${sidebarOpen ? "fixed inset-3 z-50 flex" : "hidden"} min-h-0 min-w-0 flex-col rounded-[1.15rem] border border-white/70 bg-white/56 shadow-[0_30px_90px_rgba(30,64,175,0.18)] backdrop-blur-2xl lg:relative lg:inset-auto lg:z-auto lg:flex`}>
            ...
            <div className="grid shrink-0 gap-2 border-t border-white/60 p-3"><div className="rounded-2xl border border-white/70 bg-white/45 p-3 shadow-inner backdrop-blur-xl"><div className="mb-2 flex items-center gap-2 text-xs font-medium text-blue-950"><Cloud className="h-4 w-4 text-blue-700" aria-hidden="true" />Storage</div><div className="h-2 overflow-hidden rounded-full bg-blue-100/70"><div className="h-full w-[24%] rounded-full bg-blue-500" /></div><p className="mt-2 text-xs text-slate-500">Mailbox storage available</p></div><div className="flex items-center gap-3 rounded-2xl border border-white/70 bg-white/45 p-3 backdrop-blur-xl lg:hidden"><Avatar name={displayName} /><div className="min-w-0"><p className="truncate text-sm font-semibold text-blue-950">{displayName}</p><p className="truncate text-xs text-slate-500">{mailbox || "Mailbox"}</p></div></div></div>
          </aside>
          <section className={`${mobileDetail ? "hidden lg:flex" : "flex"} min-h-0 min-w-0 flex-col rounded-[1.15rem] border border-white/70 bg-white/50 shadow-[0_24px_80px_rgba(30,64,175,0.14)] backdrop-blur-2xl`}>
            <div className="border-b border-white/60 p-3 sm:p-4"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-3"><h1 className="truncate text-lg font-semibold text-blue-950">{folder?.name || activeFolder}</h1><span className="rounded-full bg-blue-100/80 px-3 py-1 text-sm font-medium text-blue-700">{unreadCount} unread</span></div><p className="mt-1 text-sm text-slate-500">{loading ? "Loading your mailbox..." : `${page.total} messages`}</p></div><button aria-label="Refresh mailbox" className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl border border-blue-100 bg-white/70 text-blue-900 shadow-sm transition hover:-translate-y-0.5 hover:bg-white focus:outline-none focus:ring-2 focus:ring-blue-200" onClick={() => void refresh()} type="button"><RefreshCw className="h-4 w-4" aria-hidden="true" /></button></div><div className="relative mt-3"><Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" aria-hidden="true" /><GlassInput aria-label="Search this folder" className="h-10 rounded-2xl border-white/70 bg-white/78 pl-10 text-blue-950 placeholder:text-slate-500" onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && token) void loadMessages(token, activeFolder, 1, search, false); }} placeholder="Search this folder" value={search} /></div></div>
          ...
          <section className={`${mobileDetail ? "fixed inset-0 z-40 block overflow-y-auto bg-[linear-gradient(135deg,#edf7ff,#f8fbff)] p-3 lg:relative lg:inset-auto lg:z-auto lg:overflow-hidden lg:bg-transparent lg:p-0" : "hidden lg:block"} min-h-0 min-w-0 overflow-hidden rounded-[1.15rem] border border-white/70 bg-white/48 shadow-[0_24px_80px_rgba(30,64,175,0.14)] backdrop-blur-2xl`}>
            ...
            {selected ? <article className="flex h-full min-h-0 flex-col overflow-hidden"><header className="border-b border-white/60 p-3 sm:p-4"><div className="mb-3 flex items-center justify-between gap-2 text-blue-900"><div className="flex flex-wrap gap-2"><IconAction label="Reply" onClick={() => beginCompose("reply")}><Reply className="h-4 w-4" aria-hidden="true" /></IconAction><IconAction label="Reply all" onClick={() => beginCompose("reply-all")}><ReplyAll className="h-4 w-4" aria-hidden="true" /></IconAction><IconAction label="Archive" disabled={!archivePath} onClick={() => archivePath && void moveSelected(archivePath)}><Archive className="h-4 w-4" aria-hidden="true" /></IconAction><IconAction label="Delete" onClick={() => void deleteSelected()}><Trash2 className="h-4 w-4" aria-hidden="true" /></IconAction><IconAction label={selected.unread ? "Mark read" : "Mark unread"} onClick={() => void setRead()}><Mail className="h-4 w-4" aria-hidden="true" /></IconAction><IconAction label="Move" disabled={!customFolders.length} onClick={() => customFolders[0] && void moveSelected(customFolders[0].path)}><Folder className="h-4 w-4" aria-hidden="true" /></IconAction><IconAction label="More"><MoreHorizontal className="h-4 w-4" aria-hidden="true" /></IconAction></div><IconAction label="Star" onClick={() => void toggleStar()}><Star className={`h-5 w-5 ${selected.starred ? "fill-amber-400 text-amber-500" : ""}`} aria-hidden="true" /></IconAction></div><div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="mb-3 flex flex-wrap items-center gap-2"><h2 className="text-xl font-semibold leading-tight text-blue-950">{selected.subject}</h2><span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700">{folder?.name || activeFolder}</span></div><div className="flex items-center gap-3"><Avatar name={displayAddressName(selected.from)} /><div className="min-w-0"><p className="truncate text-sm font-semibold text-blue-950">{displayAddressName(selected.from)}</p><p className="truncate text-xs text-slate-500">to {selected.to || mailbox}</p>{selected.cc ? <p className="truncate text-xs text-slate-500">cc {selected.cc}</p> : null}</div></div></div><div className="hidden shrink-0 text-right sm:block"><time className="text-sm font-medium text-blue-900">{formatDate(selected.date)}</time><p className="mt-1 text-xs text-slate-500">{formatLongDate(selected.date)}</p></div></div></header><div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5"><div className="mx-auto max-w-[850px]"><div className="prose prose-slate prose-sm max-w-none text-blue-950 prose-a:text-blue-700 prose-blockquote:border-blue-200 prose-blockquote:bg-blue-50/60 prose-blockquote:px-4 prose-blockquote:py-2 prose-img:rounded-2xl" dangerouslySetInnerHTML={{ __html: selected.sanitizedHtml || textToHtml(selected.text) }} />{selected.attachments.length ? <div className="mt-5 grid gap-2"><h3 className="text-sm font-semibold text-blue-950">Attachments</h3>{selected.attachments.map((attachment) => <button className="flex items-center justify-between gap-3 rounded-2xl border border-blue-100 bg-white/72 p-3 text-left text-sm shadow-sm transition hover:-translate-y-0.5 hover:bg-white focus:outline-none focus:ring-2 focus:ring-blue-200" key={attachment.partId} onClick={() => void downloadAttachment(attachment)} type="button"><span className="flex min-w-0 items-center gap-3"><span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-red-100 text-red-600"><Paperclip className="h-4 w-4" aria-hidden="true" /></span><span className="min-w-0"><span className="block truncate font-medium text-blue-950">{attachment.filename}</span><span className="text-xs text-slate-500">{attachment.contentType} / {formatBytes(attachment.size)}</span></span></span><Download className="h-4 w-4 shrink-0 text-blue-700" aria-hidden="true" /></button>)}</div> : null}</div></div><footer className="shrink-0 border-t border-white/60 p-3"><div className="grid gap-3 sm:grid-cols-2"><GlassButton onClick={() => beginCompose("reply")} variant="glass"><Reply className="h-4 w-4" aria-hidden="true" />Reply</GlassButton><GlassButton onClick={() => beginCompose("forward")} variant="glass"><Forward className="h-4 w-4" aria-hidden="true" />Forward</GlassButton></div></footer></article> : null}
          </section>
        </main>
      </div>
...
function Avatar({ name }: { name: string }) { const initial = (name.trim()[0] || "M").toUpperCase(); return <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white/70 bg-gradient-to-br from-blue-100 to-slate-200 text-sm font-semibold text-blue-950 shadow-sm">{initial}</span>; }
