...
'
      <style>{`
        .jposta-email-preview { color: #0f172a; font-size: 15px; line-height: 1.55; }
        .jposta-email-preview :where(p, ul, ol, table, blockquote, pre) { margin-top: 0.7rem; margin-bottom: 0.7rem; }
        .jposta-email-preview :where(h1, h2, h3, h4) { color: #0f172a; font-weight: 700; line-height: 1.25; margin: 1rem 0 0.55rem; }
        .jposta-email-preview :where(a) { color: #1d4ed8; text-decoration: underline; text-underline-offset: 2px; }
        .jposta-email-preview :where(img) { display: inline-block; max-width: 100%; height: auto; margin: 0.75rem 0; border-radius: 0.75rem; }
        .jposta-email-preview :where(table) { max-width: 100%; border-collapse: collapse; color: #0f172a; }
        .jposta-email-preview :where(td, th) { padding: 0.35rem 0.45rem; vertical-align: top; }
        .jposta-email-preview :where(button, .button, [role="button"]) { margin: 0.4rem 0.35rem 0.4rem 0; }
      `}</style>
      ...
      <div className="relative z-10 flex h-dvh min-h-0 flex-col gap-2 overflow-hidden p-2 lg:p-2.5">
        ...
        <main className="grid min-h-0 flex-1 gap-2 overflow-hidden lg:grid-cols-[15rem_minmax(24rem,28rem)_minmax(0,1fr)] xl:grid-cols-[15.5rem_minmax(25rem,29rem)_minmax(0,1fr)]">
          <aside className={`${sidebarOpen ? "fixed inset-3 z-50 flex" : "hidden"} min-h-0 min-w-0 flex-col rounded-[1.15rem] border border-white/70 bg-white/56 shadow-[0_30px_90px_rgba(30,64,175,0.18)] backdrop-blur-2xl lg:relative lg:inset-auto lg:z-auto lg:flex`}>
            ...
            <div className="hidden items-center gap-3 border-b border-white/60 p-2.5 lg:flex"><LogoMark /><p className="text-base font-semibold text-blue-950">JPosta Mail</p></div>
            ...
            <div className="grid shrink-0 gap-2 border-t border-white/60 p-2.5"><div className="rounded-xl border border-white/70 bg-white/45 p-2.5 shadow-inner backdrop-blur-xl"><div className="mb-1.5 flex items-center gap-2 text-[11px] font-medium text-blue-950"><Cloud className="h-4 w-4 text-blue-700" aria-hidden="true" />Storage</div><div className="h-2 overflow-hidden rounded-full bg-blue-100/70"><div className="h-full w-[24%] rounded-full bg-blue-500" /></div><p className="mt-1.5 text-[11px] text-slate-500">Mailbox storage available</p></div><div className="flex items-center gap-3 rounded-xl border border-white/70 bg-white/45 p-2.5 backdrop-blur-xl lg:hidden"><Avatar name={displayName} /><div className="min-w-0"><p className="truncate text-[13px] font-semibold text-blue-950">{displayName}</p><p className="truncate text-xs text-slate-500">{mailbox || "Mailbox"}</p></div></div></div>
          </aside>
          <section className={`${mobileDetail ? "hidden lg:flex" : "flex"} min-h-0 min-w-0 flex-col rounded-[1.15rem] border border-white/70 bg-white/50 shadow-[0_24px_80px_rgba(30,64,175,0.14)] backdrop-blur-2xl`}>
            <div className="border-b border-white/60 p-2.5 sm:p-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-3"><h1 className="truncate text-base font-semibold text-blue-950">{folder?.name || activeFolder}</h1><span className="rounded-full bg-blue-100/80 px-2.5 py-0.5 text-xs font-medium text-blue-700">{unreadCount} unread</span></div><p className="mt-0.5 text-xs text-slate-500">{loading ? "Loading your mailbox..." : `${page.total} messages`}</p></div><button aria-label="Refresh mailbox" className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl border border-blue-100 bg-white/70 text-blue-900 shadow-sm transition hover:-translate-y-0.5 hover:bg-white focus:outline-none focus:ring-2 focus:ring-blue-200" onClick={() => void refresh()} type="button"><RefreshCw className="h-4 w-4" aria-hidden="true" /></button></div><div className="relative mt-2"><Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" aria-hidden="true" /><GlassInput aria-label="Search this folder" className="h-9 rounded-xl border-white/70 bg-white/78 pl-10 text-blue-950 placeholder:text-slate-500" onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && token) void loadMessages(token, activeFolder, 1, search, false); }} placeholder="Search this folder" value={search} /></div></div>
